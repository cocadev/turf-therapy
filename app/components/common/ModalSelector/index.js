...
export default class ModalSelector extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if(prevProps.selectedKey !== this.props.selectedKey){
            ...
            newState.changedItem = selectedItem._id;
            doUpdate = true;
        }
        if (doUpdate) {
    ...
    validateSelectedKey = (_id) => {
        let selectedItem = this.props.data.filter((item) => this.props.keyExtractor(item) === _id);
        let selectedLabel = selectedItem.length > 0 ? this.props.labelExtractor(selectedItem[0]) : this.props.initValue;
        let selectedKey = selectedItem.length > 0 ? _id : undefined;
        return {name: selectedLabel, _id: selectedKey}
    }
