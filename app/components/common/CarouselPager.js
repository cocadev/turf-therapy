...
export default class CarouselPager extends Component {
  ...
  componentWillMount() {
    this._panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        var parentScrollDisable = this.props.parentScrollDisable;
        parentScrollDisable();
        this._move = 0;
      },
      onPanResponderMove: (evt, gestureState) => {
        let suffix = this.props.vertical ? 'y' : 'x';
        let page = this._getPageForOffset(this._lastPos+gestureState['d' + suffix], gestureState['d' + suffix]);
        var pos = this._getPosForPage(page);
        // console.log('-- pos::curpos : ', pos, this._lastPos + gestureState['d' + suffix], page*this._boxSizeInterval);
        
        if( this._lastPos+gestureState['d' + suffix] > 0 || this._lastPos+gestureState['d' + suffix] < -(this.props.children.length-1)*this._boxSizeInterval ){
          // var parentScrollEnable = this.props.parentScrollEnable;
          // parentScrollEnable();
        } else {
          this._move = gestureState['d' + suffix]          
        }
        this.state.pos.setValue(this._lastPos + this._move);
      },
      ...
      onPanResponderTerminate: (evt, gestureState) => {
        ...
        this.state.pos.setValue(this._lastPos);
        var parentScrollEnable = this.props.parentScrollEnable;
        parentScrollEnable();
      },
  ...
  render() {
    var pageNum = this.props.children.length;
    var pageBtnArr = [];
    for( var i=0; i<pageNum; i++ ){
      var pageBtn = <View style={{width: 8, height: 8, borderRadius: 5, borderColor: '#fff', borderWidth: 1, marginTop: 5, backgroundColor: 'transparent'}}></View>;
      if( i == this.state.currentPage ){
        pageBtn = <View style={{width: 8, height: 16, borderRadius: 5, borderColor: '#fff', borderWidth: 1, marginTop: 5, backgroundColor: '#fff'}}></View>;
      }
      pageBtnArr.push(
        pageBtn
      )
    ...
    if (!this.state.width && !this.state.height) {
      ...
      return (
        <View style={{ flex: 1 }}>
          ...
          />
          <View style={{position: 'absolute', right: 15, top: 50, width: 14, height: 100, flexDirection: 'column', justifyContent: 'center', zIndex: 100000000}}>
            {pageBtnArr}
          </View>
        </View>
      );
